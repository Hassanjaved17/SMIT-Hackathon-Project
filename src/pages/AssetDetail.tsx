...
import {
  ...
  updateIssueStatus, assignTechnician, markOutOfService,
  createMaintenanceRecord, updateAssetServiceDates,
} from '../lib/store';
...
export default function AssetDetail() {
  ...
  async function refresh() {
    if (!id) return;
    setLoading(true);
    const [a, i, h] = await Promise.all([getAsset(id), listIssues(id), listHistory(id)]);
    setAsset(a);
    setIssues(i);
    setHistory(h);
    setLoading(false);
  ...
  if (loading) return <div className="mx-auto max-w-5xl px-6 py-16 text-steel-300">Loading…</div>;
  if (!asset) return <div className="mx-auto max-w-5xl px-6 py-16 text-steel-300">Asset not found.</div>;

  const publicUrl = `${window.location.origin}/a/${asset.code}`;

  return (
    <div className="mx-auto max-w-6xl px-6 py-10">
      <Link to="/dashboard" className="mb-6 inline-flex items-center gap-1.5 text-sm text-steel-300 hover:text-mist-100">
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="mb-2 flex items-center gap-3">
            <h1 className="font-display text-2xl font-semibold text-mist-100">{asset.name}</h1>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="space-y-8 lg:col-span-2">
          <Plate>
            <h2 className="mb-4 font-display text-base font-semibold text-mist-100">Issues on this asset</h2>
            {issues.length === 0 && <p className="text-sm text-steel-300">No issues reported yet.</p>}
          ...
          <Plate>
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="font-display text-base font-semibold text-mist-100">Asset history</h2>
            </div>
            {history.length === 0 && <p className="text-sm text-steel-300">No history recorded yet.</p>}
            <ol className="space-y-4 border-l border-graphite-700 pl-4">
              {history.map((h) => (
                <li key={h.id} className="relative">
                  ...
                  <p className="mt-0.5 text-xs text-steel-500">{h.actor} · {new Date(h.created_at).toLocaleString()}</p>
                </li>
              ))}
            </ol>
          </Plate>
        ...
        <div className="space-y-6">
          ...
          <Plate className="text-sm text-steel-300">
            <p className="mb-2 font-display text-sm font-semibold text-mist-100">Print-ready label</p>
            <p>Includes organization name, asset name, code, location, QR, and scan instructions — use Download above and print at your facility.</p>
          </Plate>
        </div>
      </div>
...
function IssueRow({
  issue, canManage, onOpenMaintenance, onChanged,
}: { issue: Issue; canManage: boolean; onOpenMaintenance: () => void; onChanged: () => void }) {
  const { user } = useAuth();
  ...
  async function handleAssign() {
    ...
    await assignTechnician(issue, techName.trim(), user?.name ?? 'Administrator');
    setTechName('');
    onChanged();
  }
  ...
  return (
    <div className="rounded-md border border-graphite-700 p-4">
      ...
      <h3 className="mt-2 font-display text-sm font-semibold text-mist-100">{issue.title}</h3>
      <p className="mt-1 text-sm text-steel-300">{issue.description}</p>
      {issue.ai_possible_causes?.length > 0 && (
        <p className="mt-2 text-xs text-steel-500">Possible causes: {issue.ai_possible_causes.join(', ')}</p>
      )}
      <p className="mt-1 text-xs text-steel-500">Reported by {issue.reporter_name} · {new Date(issue.created_at).toLocaleString()}</p>
      {issue.assigned_technician && <p className="mt-1 text-xs text-teal-400">Assigned to {issue.assigned_technician}</p>}
...
function MaintenanceModal({
  issue, technician, onClose, onSaved,
}: { issue: Issue; technician: string; onClose: () => void; onSaved: () => void }) {
  const { user } = useAuth();
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    setError(null);
    if (!notes.trim()) { setError('A maintenance note is required to resolve an issue.'); return; }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-graphite-950/80 px-4">
      <div className="plate w-full max-w-md max-h-[85vh] overflow-y-auto p-6">
        <h2 className="mb-1 font-display text-lg font-semibold text-mist-100">Resolve {issue.issue_number}</h2>
        <p className="mb-5 text-sm text-steel-300">A maintenance note is required before an issue can be resolved.</p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <Field label="Maintenance notes">
            <textarea className={`${inputClass} min-h-[90px]`} value={notes} onChange={(e) => setNotes(e.target.value)} required />
          </Field>
          <Field label="Parts used">
            <input className={inputClass} value={parts} onChange={(e) => setParts(e.target.value)} placeholder="e.g. HDMI cable" />
          </Field>
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Time (minutes)">
              <input type="number" min={0} className={inputClass} value={minutes} onChange={(e) => setMinutes(e.target.value)} />
            </Field>
          </div>
          <Field label="Final condition">
            <select className={inputClass} value={finalCondition} onChange={(e) => setFinalCondition(e.target.value)}>
              <option>Good</option><option>Fair</option><option>Poor</option>
            </select>
          </Field>
          {error && <p className="text-sm text-danger-500">{error}</p>}
          <div className="flex gap-3 pt-1">
            <Button type="submit" variant="primary" disabled={saving} className="flex-1">{saving ? 'Saving…' : 'Resolve issue'}</Button>
...
function ServiceDatesCard({ asset, onUpdated }: { asset: Asset; onUpdated: () => void }) {
  ...
  return (
    <Plate>
      <h2 className="mb-4 font-display text-base font-semibold text-mist-100">Service schedule</h2>
      <div className="grid grid-cols-2 gap-4">
        <Field label="Last service date">
          <input type="date" className={inputClass} value={last} onChange={(e) => setLast(e.target.value)} />
        </Field>
        <Field label="Next service date">
          <input type="date" className={inputClass} value={next} onChange={(e) => setNext(e.target.value)} />
        </Field>
      </div>
      {error && <p className="mt-3 text-sm text-danger-500">{error}</p>}
      <Button variant="secondary" onClick={save} disabled={saving} className="mt-4">{saving ? 'Saving…' : 'Save schedule'}</Button>
    </Plate>
  );
}
